...
def play_match(board_init, first_is_p1):
    ...
    while True:
        ...
        if end:
            w_agent = None
            if winner != -1:
                if first_is_p1: w_agent = "Minimax" if winner == 1 else "Heur"
                else: w_agent = "Minimax" if winner == 2 else "Heur"
            
            t1 = np.mean(move_times[1])*1000 if move_times[1] else 0
            t2 = np.mean(move_times[2])*1000 if move_times[2] else 0
            times = {"Minimax": t1 if first_is_p1 else t2, "Heur": t2 if first_is_p1 else t1}
...
def run():
    ...
    case_a = {"Minimax": 0, "Heur": 0, "Draws": 0}
    case_b = {"Minimax": 0, "Heur": 0, "Draws": 0}
    all_times = {"Minimax": [], "Heur": []}

    print(f"Starting: Minimax vs Heuristic MCTS ({len(second_moves)*2} games)")
    ...
    for second_idx in tqdm(second_moves):
        ...
        all_times["Minimax"].append(t["Minimax"]); all_times["Heur"].append(t["Heur"])
...
"""
=== TOTAL RUNTIME: 247.80 s ===
Average Minimax decision time: 643.81 ms
Average Heuristic MCTS decision time: 2551.85 ms
