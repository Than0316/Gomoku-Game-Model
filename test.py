#!/usr/bin/env python3
"""
Compare AlphaZero-guided MCTS vs Guided MCTS over all openings, timing each move,
and show each move with 1-second pause.
"""
...
# Flexible imports
try:
    from Monte_Carlo_guided_GOMOKU.mcts_game import Game, Board as MCTSBoard
except Exception:
    from mcts_game import Board as MCTSBoard
    from mcts_game import Game
...
def all_valid_second_moves_for_opponent(size, max_dist=2):
    """Return all valid second moves for the opponent (O) around the center."""
    ...
    for r in range(size):
        for c in range(size):
            ...
            if idx == center_idx:
                continue
            if within_chebyshev(center_idx, idx, size, max_dist=max_dist):
                moves.append(idx)
    return moves
...
def play_alphazero_vs_guided(board_init, first_is_alpha=True, n_playouts=MCTS_PLAYOUTS):
    ...
    while True:
        ...
        # --- Show move and board ---
        r, c = idx_to_rc(move_idx, size)
        print(f"Player {current} plays: ({r}, {c})")
        game_view.graphic(board, 1, 2)
        time.sleep(1)
...
def run_all_openings_vs_alpha_and_guided(n_playouts=MCTS_PLAYOUTS):
    ...
    n_in_row = 4
    center_idx = center_index(size)
    second_moves = all_valid_second_moves_for_opponent(size, max_dist=2)
